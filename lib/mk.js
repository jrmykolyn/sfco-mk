...
function _writeFile( data ) {
	var fileName = _getFileName( MANIFEST[ SETTINGS.VARIANT || 'default' ] );

	return new Promise( ( resolve, reject ) => {
		literati.write( `${process.cwd()}/${fileName}`, data )
			.then( () => {
...
function _returnOverview( data ) {
	return new Promise( ( resolve, reject ) => {
		resolve( `Created new ${data.fileName || ''} file at ${process.cwd()}` );
	} );
}
